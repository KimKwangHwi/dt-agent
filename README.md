...
세부적인 구조도나 동작 방식은 251212 업무보고 파일을 참고바랍니다.
...
3.  **프론트엔드 접근:**
    백엔드가 실행된 후, 웹 브라우저에서 `frontend/index.html` 파일을 열거나, 웹 서버를 통해 접근합니다.

## API 호출 기반 LLM 관련 
    - LLM API key의 경우 .env 파일을 추가 후 직접 선언해야 합니다.
    - LLM API 호출은 외부망을 사용해야 합니다.